...
//	if(worker->knob_index % ((int)(1/worker->scale)) != 0){	
	return;
      }
//	else{
//		worker->knob_index = 0;
...
//else{
 //	int meanInterarrival = 1.0/((400000)/(float)config->n_workers)*1e6;
   //     interarrival_time = worker->scale * meanInterarrival; //In microseconds
...
void createWorkers(struct config* config) {
  ...
  for(i = 0; i < config->n_workers; i++) {
    ...
    config->workers[i]->scale = config->server_share[server];
    int rc;
